...
/* class Color - r, g, b, a values for defined colors */
class Color {
    public:
        float r, g, b;
...
/* Array of colors used in 1st stage of explosion */
...
const int PARTICLES_COLORS_1_SIZE = 15;

/* Array of colors used in 2nd stage of explosion */
...
const int PARTICLES_COLORS_2_SIZE = 15;

/* Array of colors used in 3rd (last) stage of explosion */
...
const int PARTICLES_COLORS_3_SIZE = 15;

/*
    class ParticleValues - auxiliary class with partition's values.
    Required for creating explosion pattern vector.
 */
...
/*
    class Particle - represents single particle
*/
class Particle : public ParticleValues, public Object {
    private:
        ...
        void setColor(int stage) { // sets color for each explosion's stage
            srand(time(0));
            Color tempColor;
    ...
    public:
        ...
        void setValues(ParticleValues p) { // initalises Particle with values from pattern
            this->startX = p.startX;
            this->startY = p.startY;
            this->startZ = p.startZ;
        ...
        void initMove(float speedScale) { // makes the first move in explosion
            this->moveLocation(speedX*speedScale, speedY*speedScale, speedZ*speedScale);
        }

        void drawObject() // draws particle without any moves or rotates
        {
            if(ttl == 0) return;
            ...
            glDepthMask(false); // required for alpha transparecy
            shader->use();
        ...
        void drawObject2() // makes next move, updates color and draws particle
        {
            if(ttl == 0) return;
...
/*
    class Explosion - Group of all particles included in explosion
*/
class Explosion : public ObjectGroup {
    ...
    public:
        ...
        void kaboom(float x, float y, float z, float delayTime) {
            ...
            if(direction.z == 0 || abs(direction.z) < epsilon) {
                this->setAngle2(0, -90, 0);
            }
            else {
                this->setAngle2(0,radiansToDegrees(atan(direction.x/direction.z)),0);
            }
